 ...
 * Integra con /src/services/countriesService.js - SIN DUPLICACIÓN
...
function obtenerNombrePais(codigoPais) {
    ...
    // ✅ CORRECCIÓN: Verificar window.countriesService en lugar de countriesService
    if (typeof window.countriesService !== 'undefined' && window.countriesService && window.countriesService.obtenerNombrePais) {
...
async function cargarPaises() {
    try {
        ...
        if (paises && paises.length > 0) {
            ...
            const paisesObjeto = {};
            paises.forEach(pais => {
                // Asumiendo que la API devuelve objetos con propiedades como 'code' y 'name'
                if (pais.code) {
                    paisesObjeto[pais.code.toLowerCase()] = pais.name || pais.nombre;
                } else if (pais.id) {
                    // Si usa 'id' en lugar de 'code'
                    paisesObjeto[pais.id.toLowerCase()] = pais.name || pais.nombre;
                }
            });
            ...
            console.warn('⚠️ No se recibieron países desde la API');
            ...
            window.countriesCache = fallbackPaises;
            return Object.keys(fallbackPaises).map(code => ({ code, name: fallbackPaises[code] }));
        }
    } catch (error) {
